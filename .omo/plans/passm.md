...
- [ ] 7. Android cross-compile spike: git2 + keyring for aarch64-linux-android (TOP RISK, fail fast)
...
- [ ] 9. passm-sync: git plumbing (ensure_clone / fetch / fast-forward push / non-FF detection)
...
- [ ] 10. passm-sync: conflict merge (backup-before-merge + converge + retry)
...
- [ ] 11. Tauri app shell: window, tray, auto-lock timer, session state
...
- [ ] 12. Backend commands (unlock/lock/CRUD/search/copy/generate/sync/config)
...
- [ ] 13. Frontend: React 18 + TS + Vite (unlock, list/search, editor, copy, sync status)
