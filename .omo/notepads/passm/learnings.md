...
## [2026-08-18] User has real GUI access to the host machine
- User CAN operate the host machine's real GUI desktop terminal (not just SSH). They offered: "I can also control it from the host GUI terminal — tell me if you need anything."
- Implication: F3 manual QA can be done on the REAL desktop by the user (best fidelity — real window, real tray, real keyboard focus). T14 headless smoke via Xvfb (subagent in progress) still fine for logical IPC verification; real visual/feel verification goes to user-assisted F3.
- Ask user for: real desktop `tauri dev` run + screenshots at F3; Windows MSI install test + Android APK install test at F4.

## [2026-08-18] T16 Android APK — hard-won build environment lessons
- Memory is tight on this box (3.8Gi total, ~1.6Gi available with opencode running). Long cargo/gradle builds get OOM-killed. Mitigations: `CARGO_BUILD_JOBS=2`, gradle jvmargs 1024m, run via `setsid nohup ... & disown` (plain nohup+& dies when the bash tool session ends).
- `tauri android build` internally invokes `cargo build --target <abi>` WITHOUT cargo-ndk's env setup. openssl-sys's make needs `<triple>-ranlib` etc. on PATH. NDK r26d only ships llvm-* tools → create symlinks: for each ABI triple (aarch64-linux-android, armv7-linux-androideabi, i686-linux-android, x86_64-linux-android): ln -sf llvm-{ar,ranlib,strip,nm,objdump,readelf} <triple>-<tool>, plus <triple>-clang/++ and ld. Also export CC_/AR_/RANLIB_<triple>= llvm paths for the first build.
- The gradle rustBuild* task runs `node tauri android android-studio-script` with workingDir = src-tauri/. `src-tauri/tauri` must exist as a symlink to `../node_modules/@tauri-apps/cli/tauri.js` — `tauri android init` normally creates it; ours was missing (likely not created/committed by the T16 subagent before timeout). A node shim file did NOT work (project is "type": "module" → require() throws) — use the symlink.
- gradle-8.14.3-bin.zip must be present at ~/.gradle/wrapper/dists/.../gradle-8.14.3-bin.zip. The gradle wrapper's own download failed (Java SSL handshake) AND curl to services.gradle.org→GitHub CDN failed (HTTP=000). User manually downloaded the zip; curl from a different mirror worked for them. Lesson: if wrapper download fails, have the user fetch gradle-8.14.3-bin.zip.
- openssl-src on x86_64-linux-android misconfigures as linux-x86_64 → SM3/SM4 AVX512 asm (vsm3msg1 etc.) fails to assemble. FIX: build only arm64: `tauri android build --apk -t aarch64` (modern devices are arm64; universal APK unnecessary).
- Tauri release buildType has no signingConfig; `tauri android build --apk` output is UNSIGNED (app-universal-release-unsigned.apk). Sign manually: keytool -genkeypair (debug.keystore, android/android) then `apksigner sign --ks ~/.android/debug.keystore --ks-pass pass:android --out app-universal-release.apk app-universal-release-unsigned.apk`; verify with `apksigner verify --print-certs`.
- `tauri android build` warns about identifier "com.passm.app" ending in .app (macOS conflict) — harmless for Android; leave as-is (F4 may note it).
- App's #[cfg_attr(mobile, tauri::mobile_entry_point)] triggers unused_must_use warning on android target builds — cosmetic, present in Tauri template too.

## [2026-08-19] APK 闪退修复 — 缺 libandroid_native_keyring_store.so
- 症状: 用户手机(红米K60)安装 APK 打开即闪退。根因: `MainActivity.onCreate` → `Keyring.initializeNdkContext` → Keyring companion init `System.loadLibrary("android_native_keyring_store")` → `UnsatisfiedLinkError`。APK 只含 `libpassm_app_lib.so`, keyring store 的 cdylib 从未被打包(它是独立 crate 产物, 在 target/.../deps/libandroid_native_keyring_store-*.so, 不会自动进 APK)。
- 修复: `cp target/aarch64-linux-android/release/deps/libandroid_native_keyring_store-*.so → src-tauri/gen/android/app/src/main/jniLibs/arm64-v8a/libandroid_native_keyring_store.so`。jniLibs/arm64-v8a 里 tauri 已放好 `libpassm_app_lib.so` 的符号链接, 手动 .so 放同目录即可被打包。APK 体积 15.6MB (含两个 .so)。
- 附带修复: MainActivity.kt 中 `enableEdgeToEdge()` 原在 `super.onCreate()` 之前 → 移到之后 (标准模板顺序)。
- 构建坑: 直接 `./gradlew :app:assembleUniversalRelease` 会 panic "failed to read missing addr file /tmp/com.passm.app-server-addr" (tauri-cli 上下文缺失) → 必须走 `./node_modules/.bin/tauri android build --apk -t aarch64`。
- openssl-sys 重编译时 `make install_dev` 报 `aarch64-linux-android-ranlib: not found` → NDK r26d 只带 llvm-* 工具; 需 ln -sf llvm-{ar,ranlib,nm,strip,objcopy,objdump,readelf,size,strings,addr2line,c++filt} aarch64-linux-android-<tool> + aarch64-linux-android-ld→ld.lld, 且 NDK bin 要进 PATH(export PATH=...llvm/prebuilt/linux-x86_64/bin:$PATH) — gradle daemon 缓存旧环境, 改完 PATH 后需先 kill daemon。
- 签名不变 (debug.keystore, SHA-256 365fe2c8...dd843) → 用户可直接覆盖安装。

## [2026-08-19] ensure_cert_store fix: stale cacert.pem DIRECTORY → OpenSSL ERR_R_BIO_LIB (EISDIR)
- Real-device failure: `OpenSSL error: failed to load certificates: error:05880020:x509 certificate routines::BIO lib` on Android sync. Root cause: `Path::exists()` returns true for a DIRECTORY too — a stale `cacert.pem` directory (or empty/corrupt file) left by an earlier buggy build skipped the `if !cert_path.exists()` rewrite guard, then libgit2's `set_ssl_cert_file` → `SSL_CTX_load_verify_locations` → `X509_load_cert_crl_file_ex` → `BIO_new_file` (fopen) failed with EISDIR. `fs::write` never errored because it was never reached.
- Fix (git_repo.rs `ensure_cert_store`): (1) if `cert_path.is_dir()` → `remove_dir_all`; (2) UNCONDITIONALLY `fs::write` the bundle (drop the exists() guard — always-rewrite is idempotent and cheap, 182KB); (3) read back + byte-compare, retry once (delete+rewrite) on mismatch, else `SyncError::Git(from_str(...))` with path + expected/actual lengths; (4) enrich `set_ssl_cert_file` errors with `cert_path.display()`. New tests: `ensure_cert_store_overwrites_stale_empty_file` + `ensure_cert_store_replaces_stale_directory` (the EISDIR repro). Suite: 33 → 35 passm-sync tests.
- Lesson: `Path::exists()` conflates files and directories — when a path must be a readable FILE, never gate a rewrite on `exists()`; check `is_dir()` explicitly and verify bytes after writing.

## [2026-08-20] Android「保存中…」永久转圈 ROOT CAUSE — 双 .so ndk-context 重复静态量
- 症状: 红米K60 上设置同步配置输入地址+token 后永远「保存中…」。logcat 抓到: `thread 'tokio-rt-worker' panicked at ndk-context-0.1.1/src/lib.rs:72:30: android context was not initialized`。panic 发生在 tokio 工作线程 → Tauri 命令永不返回 → 前端永久转圈。
- 根因: 应用里有**两个 .so，各自静态链接一份 ndk-context crate，各有独立的 ANDROID_CONTEXT 静态量**:
  1. `libandroid_native_keyring_store.so`(独立 cdylib, jniLibs 里的 414KB) — MainActivity 调 `Keyring.initializeNdkContext` 时 JNI 解析到**这个** .so 的符号, 初始化的是它自己那份 ANDROID_CONTEXT。
  2. `libpassm_app_lib.so`(主库, 静态链接 store 的 rlib) — Rust 侧 `android_native_keyring_store::Store::new()` 读的是**主库自己**那份 ANDROID_CONTEXT, **从未被初始化** → panic。
- 验证: `nm -D` 证明两个 .so 都导出 `Java_io_crates_keyring_Keyring_00024Companion_initializeNdkContext`, 且主库内含 "android context was not initialized" 字符串。android-native-keyring-store-1.0.0 Cargo.toml crate-type=["lib","cdylib"] 同时以两种方式链接。ndk-context 用 `static mut ANDROID_CONTEXT`(lib.rs:23), 每个 .so 独立一份。
